...
sub("<script src=\"efemerides.js\"></script>  <!-- lista de informes del menú Efemérides; persiste entre versiones -->",
    "<script src=\"efemerides.js\"></script>  <!-- lista de informes del menú Efemérides; persiste entre versiones -->\n<script src=\"observacion.js\"></script>  <!-- temperatura de ahora mismo; lo regenera el robot cada hora. Si falta, el panel simplemente no aparece -->",
    "script de observación")
sub("  <main>\n    <div class=\"chart-wrap\">",
    "  <main>\n    <!-- Panel de observación en tiempo real; lo rellena pintarObservacion() si hay datos -->\n    <div class=\"obs-panel\" id=\"obs-panel\" hidden></div>\n    <div class=\"chart-wrap\">",
    "panel en el main")
...
sub("let curCompareYears = new Set();",
    "let curCompareYears = new Set();\n// Modo comparar: cada toque añade o quita un año. En pantallas táctiles va\n// activado de salida, porque ahí no existe Ctrl+clic y tocar una etiqueta para\n// añadirla es lo que se espera; con ratón se mantiene clic = un año.\nconst ES_TACTIL = !!(window.matchMedia && window.matchMedia('(hover: none) and (pointer: coarse)').matches);\nlet cmpMode = ES_TACTIL;\nlet ultimoDiaTocado = null;   // en táctil, el mapa sinóptico se abre al segundo toque\n\n// ---- Panel \"ahora mismo\" ------------------------------------------------------\n// Observación horaria de AEMET, SIN validar y aparte de la serie de la gráfica,\n// que usa los valores climatológicos diarios (validados, 2-4 días de retraso).\n// Si observacion.js no existe (uso en local sin descargarlo), el panel no aparece.\nfunction pintarObservacion(){\n  const p = document.getElementById('obs-panel');\n  if(!p) return;\n  const O = window.OBSERVACION;\n  if(!O || !Array.isArray(O.estaciones) || !O.estaciones.length){ p.hidden = true; return; }\n  const L = (document.documentElement.lang === 'fr');\n  const hora = s => { try{ return new Date(s).toLocaleTimeString(L?'fr-FR':'es-ES',\n                        {hour:'2-digit', minute:'2-digit', timeZone:'Europe/Madrid'}); }\n                      catch(e){ return ''; } };\n  const el = (tag, cls, txt) => { const d = document.createElement(tag);\n    if(cls) d.className = cls; if(txt != null) d.textContent = txt; return d; };\n\n  p.replaceChildren();\n  const head = el('div','obs-head');\n  head.appendChild(el('h2', null, '🌡️ ' + (L?'En ce moment':'Ahora mismo')));\n  const ult = O.estaciones.map(e => e.fint).filter(Boolean).sort().pop();\n  if(ult) head.appendChild(el('div','obs-when',(L?'mesuré à ':'medido a las ') + hora(ult)));\n  p.appendChild(head);\n\n  const cards = el('div','obs-cards');\n  O.estaciones.forEach(e => {\n    const c = el('div','obs-card');\n    c.style.setProperty('--c', e.color || '#999');\n    c.appendChild(el('div','obs-name', e.short));\n    // La ubicación de AEMET a veces repite el nombre de la estación (\"Murcia / Murcia\")\n    if(e.ubi && e.ubi.toLowerCase() !== (e.short||'').toLowerCase())\n      c.appendChild(el('div','obs-ubi', e.ubi));\n    c.appendChild(el('div','obs-temp', e.ta != null ? e.ta.toFixed(1) + ' °C' : '—'));\n    const mm = [];\n    if(e.tamax != null) mm.push((L?'max ':'máx ') + e.tamax.toFixed(1) + ' °C');\n    if(e.tamin != null) mm.push((L?'min ':'mín ') + e.tamin.toFixed(1) + ' °C');\n    if(mm.length) c.appendChild(el('div','obs-mm', mm.join('  ·  ') +\n      (e.horas ? '   (' + (L?'dernières ':'últimas ') + e.horas + ' h)' : '')));\n    const ex = el('div','obs-extra');\n    if(e.hr != null) ex.appendChild(el('span', null, '💧 ' + e.hr + ' %'));\n    if(e.vv != null) ex.appendChild(el('span', null, '💨 ' + e.vv.toFixed(0) + ' km/h' +\n      (e.vmax != null ? ' (' + (L?'raf. ':'racha ') + e.vmax.toFixed(0) + ')' : '')));\n    if(e.prec != null) ex.appendChild(el('span', null, '🌧️ ' + e.prec.toFixed(1) + ' mm'));\n    if(ex.childNodes.length) c.appendChild(ex);\n    cards.appendChild(c);\n  });\n  p.appendChild(cards);\n  p.appendChild(el('div','obs-nota', L\n    ? \"Observation non validée d'AEMET. Le graphique utilise les valeurs quotidiennes officielles, publiées avec 2 à 4 jours de retard.\"\n    : 'Observación sin validar de AEMET. La gráfica usa los valores diarios oficiales, que se publican con 2–4 días de retraso.'));\n  p.hidden = false;\n}\n// Recarga el archivo con antimemoria, para que la página abierta se mantenga al día\nfunction refrescarObservacion(){\n  const s = document.createElement('script');\n  s.src = 'observacion.js?t=' + Date.now();\n  s.onload = () => { try{ pintarObservacion(); }finally{ s.remove(); } };\n  s.onerror = () => s.remove();\n  document.head.appendChild(s);\n}\n\n// ---- Tooltip del gráfico en HTML --------------------------------------------\n// El tooltip de Chart.js se dibuja en canvas: no admite negritas, color ni\n// columnas alineadas. Lo sacamos a HTML para poder jerarquizar la información.\n// El callback label() marca las líneas con TT_SEP; las que no llevan marca se\n// pintan tal cual, así los gráficos que no la usan siguen funcionando igual.\nconst TT_SEP = '\\x1f';\nfunction renderChartTooltip(ctx){\n  const {chart, tooltip} = ctx;\n  const wrap = chart.canvas.parentNode;\n  let el = wrap.querySelector('.cjs-tt');\n  if(!el){ el = document.createElement('div'); el.className = 'cjs-tt'; wrap.appendChild(el); }\n  if(tooltip.opacity === 0){ el.style.opacity = 0; return; }\n  try{\n    const frag = document.createDocumentFragment();\n    const add = (cls, txt) => { const d = document.createElement('div'); d.className = cls;\n      if(txt != null) d.textContent = txt; frag.appendChild(d); return d; };\n    const par = (padre, txt, peso) => { const s = document.createElement('span');\n      s.textContent = txt; if(peso) s.style.fontWeight = peso; padre.appendChild(s); return s; };\n    (tooltip.title || []).forEach(t => add('tt-date', t));\n    (tooltip.body || []).forEach((b, i) => {\n      const col = (tooltip.labelColors || [])[i] || {};\n      (b.lines || []).forEach((linea, j) => {\n        const s = String(linea);\n        // Primera línea sin marca = nombre de la serie, con su trazo de color\n        if(!s.startsWith(TT_SEP)){\n          if(j > 0){ add('tt-plain', s); return; }\n          const h = add('tt-serie');\n          const k = document.createElement('i'); k.className = 'tt-key';\n          k.style.background = col.borderColor || col.backgroundColor || 'currentColor';\n          h.appendChild(k); par(h, s);\n          return;\n        }\n        const p = s.split(TT_SEP);   // p[0] siempre vacío\n        if(p[1] === 'S'){   // cabecera de serie con su valor a la derecha\n          const h = add('tt-serie tt-serie-val');\n          const k = document.createElement('i'); k.className = 'tt-key';\n          k.style.background = col.borderColor || col.backgroundColor || 'currentColor';\n          h.appendChild(k); par(h, p[2]);\n          const val = par(h, p[3]); val.className = 'tt-serie-num';\n        }\n        else if(p[1] === 'C'){ add('tt-cap', p[2]); }\n        else if(p[1] === 'H'){ add('tt-hero', p[3]); add('tt-hero-lbl', p[2]); }\n        else if(p[1] === 'D'){ add('tt-badge ' + (p[3] === 'up' ? 'up' : 'down'),\n                                   (p[3] === 'up' ? '▲ ' : '▼ ') + p[2]); }\n        else if(p[1] === 'R'){ const r = add('tt-row'); par(r, p[2]); par(r, p[3]); }\n        else if(p[1] === 'G'){\n          const tn = +p[2], tx = +p[3], nv = (p[4] === '' ? null : +p[4]), color = p[5] || 'currentColor';\n          const lo = Math.min(tn, nv == null ? tn : nv), hi = Math.max(tx, nv == null ? tx : nv);\n          const m = (hi - lo) * 0.12 || 1, a = lo - m, z = hi + m;\n          const pc = x => ((x - a) / (z - a)) * 100;\n          const bar = add('tt-bar');\n          const seg = document.createElement('i');\n          seg.style.left = pc(tn) + '%';\n          seg.style.width = Math.max(pc(tx) - pc(tn), 2) + '%';\n          seg.style.background = color;\n          bar.appendChild(seg);\n          if(nv != null){ const t = document.createElement('b');\n            t.style.left = 'calc(' + pc(nv) + '% - 1px)'; bar.appendChild(t); }\n          // Los extremos nombran mín y máx; el del medio dice qué es la marca amarilla\n          const ends = add('tt-ends'); par(ends, p[6] || '');\n          if(p[8]){ const c = par(ends, p[8]); c.style.color = '#c98a00'; c.style.fontWeight = '700'; }\n          par(ends, p[7] || '');\n        }\n      });\n    });\n    (tooltip.footer || []).forEach(t => { if(String(t).trim()) add('tt-foot', t); });\n    el.replaceChildren(frag);\n  }catch(err){\n    // Nunca dejar al usuario sin tooltip por un fallo de formato.\n    // Sin depender de nada externo: si el respaldo fallara, no habría tooltip.\n    try{\n      el.textContent = (tooltip.body || []).map(b => (b.lines || []).join(' · '))\n                         .join('  |  ').split('\\x1f').join(' ');\n    }catch(_){ el.textContent = ''; }\n  }\n  const w = el.offsetWidth, h = el.offsetHeight;\n  let x = tooltip.caretX + 16;\n  if(x + w > chart.canvas.clientWidth) x = tooltip.caretX - w - 16;\n  el.style.left = Math.max(0, x) + 'px';\n  el.style.top = Math.max(0, Math.min(tooltip.caretY - h / 2, chart.canvas.clientHeight - h)) + 'px';\n  el.style.opacity = 1;\n}",
    "estado táctil + panel + tooltip HTML")
sub("  buildStationList();\n  if(typeof buildEfemPanel === 'function') buildEfemPanel();",
    "  pintarObservacion();\n  setInterval(refrescarObservacion, 15 * 60 * 1000);   // la observación se renueva cada hora\n  // En táctil el modo comparar arranca encendido: reflejarlo en el botón y en la ayuda\n  if(ES_TACTIL){\n    const bt = document.getElementById('cmp-toggle');\n    if(bt) bt.classList.add('on');\n    const ayuda = document.querySelector('#year-compare-bar .year-compare-label small span');\n    if(ayuda){\n      ayuda.textContent = 'toca años para compararlos · «➕ Varios» apagado = uno cada vez';\n      ayuda.setAttribute('data-fr', 'touchez des années pour comparer · «➕ Plusieurs» éteint = une seule');\n    }\n  }\n  buildStationList();\n  if(typeof buildEfemPanel === 'function') buildEfemPanel();",
    "arranque del panel y del modo táctil")
...
sub("        if(dateStr && typeof openSynopticModal === 'function') openSynopticModal(dateStr);",
    "        if(dateStr && typeof openSynopticModal === 'function'){\n          // En táctil no hay \"pasar el ratón\": el mismo toque mostraría el dato y\n          // abriría el mapa encima, tapándolo. El primero enseña el dato; solo un\n          // segundo toque sobre el MISMO día abre el mapa.\n          if(ES_TACTIL && ultimoDiaTocado !== dateStr){ ultimoDiaTocado = dateStr; return; }\n          ultimoDiaTocado = null;\n          openSynopticModal(dateStr);\n        }",
    "mapa sinóptico al segundo toque")
